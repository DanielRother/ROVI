...
public:
  RotaryValue();
  RotaryValue(const int maxValue, const int minValue = 0,
              const bool preventOverflow = false);
...
public:
  ...
  /*!
   * \brief Converts the transitions of the rotary encoder into a tick of the
   * counter
   *
   * As there are n (typically n=4) transitions of the encoder signal during one
   * pysical rotation tick these have to be converted before updating the
   * counter value. I.e. there have to be n transitions for one update.
   * Additionally, and to keep bouncing effects of the hardware to a minimum,
   * there is a timeout which resets the transistion counter after a while (e.g.
   * one transition was lost but the counter is still updated after m ms as n-1
   * transitions are enough to savely detect the tick).
   *
...
public:
  ...
  RotaryEncoderWithButton(const uint8_t pinA, const uint8_t pinB,
                          const uint8_t pinButton,
                          const std::string &name = "RotaryEncoder");
  // RotaryEncoderWithButton(const RotaryEncoder& other);
  virtual ~RotaryEncoderWithButton() = default;

  void update();
  void setupState(const ButtonStates state, const int minValue,
                  const int maxValue, const int increment,
  ...
  void setupState(const ButtonStates state, const int maxValue,
                  const bool preventOverflow,
                  const std::function<void(void)> stateActivatedCallback,
                  const std::function<void(int)> stateValueChangedCallback);
  ...
  ButtonStates getCurrentButtonState(); // TODO: rename to state()
  unsigned long getLastButtonStateUpdateMs() const;
  std::map<ButtonStates, ButtonStateSetting> getStateSettings() const;
  std::map<ButtonStates, RotaryValue> getStateValues() const;
...
protected:
  ...
  void invokeRotaryValueChangeCallback(const ButtonStates state,
                                       const int value);
