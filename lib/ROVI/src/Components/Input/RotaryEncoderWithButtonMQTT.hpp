...
class RotaryEncoderWithButtonMQTT
    : public RotaryEncoderWithButton,
      public MqttComponent<RotaryEncoderWithButtonMQTT> {
public:
  RotaryEncoderWithButtonMQTT(const uint8_t pinA, const uint8_t pinB,
  ...
  static const size_t MAX_SIZE =
      2500; // 1146 = 450 + 2*JSON_ARRAY_SIZE(4) + 4*JSON_OBJECT_SIZE(2) +
            // JSON_OBJECT_SIZE(4) + 4*JSON_OBJECT_SIZE(5);
};
