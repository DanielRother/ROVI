...
protected:
  ...
  std::shared_ptr<Color> m_color1; // Aus irgendeinem verfickten Grund wird
                                   // m_color nicht immer ordentlich gesetzt...
  std::shared_ptr<Color> m_color2;
  std::shared_ptr<LEDEffect> m_effect;
};

// class LEDComponentDepracedRovi : public Deprecated::RoviComponent, public
// LEDComponent /*, public std::enable_shared_from_this<LEDComponent>*/ {
// public:
//     // The convertion between string and concrete data type can be done in
//     this abstract class
//     // But the actual action have to be implemented in an derived class
//     // Names due to overloaded function and bind make problems and would led
//     to long typecasts... virtual void setPowerMQTT(const std::string&
//     payload); virtual void setColorMQTT(const std::string& payload); virtual
//     void setBrightnessMQTT(const std::string& payload); virtual void
//     setEffectMQTT(const std::string& payload);
// };

class AdressableLedComponent : public LEDComponent {
public:
  AdressableLedComponent(size_t nbPixel,
                         const std::string &name = "AdressableLedComponent");
