...
class AbsoluteHueMQTT : public MqttDevice,
                        public SimpleAbsoluteHue<ROTARY, LED> {
public:
  AbsoluteHueMQTT(Basecamp &iot, const std::shared_ptr<ROTARY> rotary,
                  ...
                  const std::string &name = "AbsoluteHueMQTT",
                  std::chrono::minutes timePerEffect = std::chrono::minutes{})
      : BasicDevice(iot),
        MqttDevice(iot), SimpleAbsoluteHue<ROTARY, LED>(
                             iot, rotary, led, effects, name, timePerEffect) {
    restoreSettings();
...
protected:
  // TODO/TBD: Reuse mqtt implementation of LedDeviceMQTT and only at rotary
  // stuff here?
  virtual void createMqttMessage(String &output) override {
    ...
    std::cout << "    send color via mqttt: " << curRgbColor->toString()
              << std::endl;
